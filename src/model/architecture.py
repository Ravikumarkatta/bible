...
class BiblicalTransformer(nn.Module):
    ...
    def __init__(self, config: BiblicalTransformerConfig):
        ...
        # Core embeddings
        self.token_embedding = TokenEmbeddings(config.vocab_size, config.hidden_size)
    ...
    def forward(
        ...
        position_embeds = self.position_embedding(seq_length, input_ids.device)
        
        # Detect and embed verse references
        verse_embeds = self.get_verse_embeddings(input_ids)
        
        # Combine embeddings
        hidden_states = token_embeds + position_embeds + verse_embeds
